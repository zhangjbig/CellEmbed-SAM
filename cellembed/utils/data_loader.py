...
def global_worker_init_fn(worker_id, base_seed=42):
...
def get_loaders(train_images_local, train_labels_local, val_images_local, val_labels_local, train_meta, val_meta, args):
    ...
    from cellembed.utils.augmentation_config import get_augmentation_dict
    from cellembed.utils.AI_utils import Segmentation_Dataset, collate_fn
    from torch.utils.data import DataLoader
    import random

    def set_seed(seed=args.rng_seed or 42):
    ...
    if args.samples:
        ...
        if args.rng_seed:
            set_seed(args.rng_seed or 42)

        if not args.weight:

            generator = torch.Generator()
            generator.manual_seed(args.rng_seed or 42)

            if args.length_of_epoch is not None:
                train_sampler = RandomSampler(train_data, num_samples=args.length_of_epoch, generator=generator)
                test_sampler = RandomSampler(test_data, num_samples=int(args.length_of_epoch * 0.2), generator=generator)  # This is relates to the standard 80/20 split
            else:
                train_sampler = RandomSampler(train_data, generator=generator)
                test_sampler = RandomSampler(test_data, generator=generator)
        else:

            if args.rng_seed:
                set_seed(args.rng_seed or 42)

            datasets_train = [meta["specimen"] for meta in train_meta]
            ...
            if args.length_of_epoch is not None:
                train_sampler = WeightedRandomSampler(rel_freq, args.length_of_epoch, generator=generator)
            else:
                train_sampler = WeightedRandomSampler(rel_freq, len(freq), generator=generator)

            print(dict_datasets)

            datasets_val = [meta["specimen"] for meta in val_meta]
            ...
            if args.length_of_epoch is not None:
                test_sampler = WeightedRandomSampler(rel_freq, int(args.length_of_epoch * 0.2), generator=generator)
            else:
                test_sampler = WeightedRandomSampler(rel_freq, len(freq), generator=generator)
        ...
        if args.rng_seed:
            ...
            train_sampler = RandomSampler(train_data, replacement=False, generator=g)
            test_sampler = RandomSampler(test_data, replacement=False, generator=g)

        else:

            train_sampler = RandomSampler(train_data, replacement=False)
            test_sampler = RandomSampler(test_data, replacement=False)
