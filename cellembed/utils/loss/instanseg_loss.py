...
from cellembed.utils.pytorch_utils import torch_fastremap, torch_onehot, fast_sparse_intersection_over_minimum_area, \
    remap_values, fast_iou, fast_sparse_iou, eccentricity_batch, connected_components, flood_fill, fill_holes, \
    expand_labels_map
...
def generate_coordinate_map(mode: str = "linear", spatial_dim: int = 2, height: int = 256, width: int = 256,
    ...
    if mode == "linear":
        if spatial_dim == 2:
            ...
            xxyy = torch.zeros((spatial_dim, height, width),
                               device=device)  # NOT IMPLEMENTED - THIS IS JUST A DUMMY VALUE
...
def feature_engineering_slow(x: torch.Tensor, c: torch.Tensor, sigma: torch.Tensor, window_size: int,
    ...
    x_slices = x[:, mesh_grid_flat[0], mesh_grid_flat[1]].reshape(E, C, 2 * window_size, 2 * window_size).permute(1, 0,
                                                                                                                  2,
                                                                                                                  3)  # C,E,2*window_size,2*window_size
    sigma_slices = sigma[:, mesh_grid_flat[0], mesh_grid_flat[1]].reshape(S, C, 2 * window_size,
                                                                          2 * window_size).permute(1,
                                                                                                   0,
                                                                                                   2,
...
class InstanSeg(nn.Module):
    ...
    def forward(self, prediction: torch.Tensor, instances: torch.Tensor, w_inst: float = 1.5, w_seed: float = 1.0):
        ...
        xxyy = generate_coordinate_map(mode="linear", spatial_dim=self.dim_coords, height=height, width=width,
        ...
        for mask_channel in range(0, instances.shape[1]):
            ...
            seed_map_batch = prediction_b[:,
                             self.dim_coords + self.n_sigma: self.dim_coords + self.n_sigma + 1]  # 1 x h x w
                ...
                if len(instance_ids) > 0:
                    ...
                    if self.multi_centre:
                        ...
                        if self.use_object_classifier:
                            ...
                            centroids = torch_peak_local_max(seed_map_tmp.squeeze() * (onehot_labels.sum(0) == 0),
                                                             neighbourhood_size=3, minimum_value=0.5).T
                            if self.to_centre:
                        ...
                        if self.to_centre:
                            ...
                            centres = xxyy.flatten(1).T[((seed_map_tmp * onehot_labels).flatten(1)).argmax(
                                1)]  # location at max seed (used in postprocessing)
                            seed_map = seed_map_tmp + seed_map_min
                        else:
                            seed_map_tmp = seed_map - seed_map.min()
                            centres = spatial_emb.flatten(1).T[((seed_map_tmp * onehot_labels).flatten(1)).argmax(
                                1)].detach()  # embedding at max seed (used in postprocessing)

                        centroids = (torch.sum(((xxyy[:2] * onehot_labels.unsqueeze(1))).flatten(2),
                                               dim=2) / onehot_labels.flatten(1).sum(1)[:, None]) * (
                    ...
                    if self.use_object_classifier:
                        if centres_neg.shape[0] > 0:
                            ...
                            labels = torch.cat((torch.ones(centres.shape[0] - centres_neg.shape[0]),
                                                torch.zeros(centres_neg.shape[0]))).to(self.device)

                            pred = get_crop_probs(spatial_emb, centres, sigma, centroids,
                                                  feature_engineering=self.feature_engineering,
                                                  object_classifier=self.object_classifier, window_size=window_size)

                            loss += torch.nn.BCEWithLogitsLoss()(pred, labels[:, None])

                    crop = onehot_labels.squeeze(1)[coords[0], coords[1], coords[2]].reshape(-1, window_size,
                                                                                             window_size)
        ...
        loss = loss / (b + 1)
        all_seed_loss = all_seed_loss / (b + 1)
        all_instance_loss = all_instance_loss / (b + 1)
        ...
        for i in range(iterations):
            ...
            if precomputed_crops is None:
                ...
                if precomputed_seeds is None:
                    local_centroids_idx = torch_peak_local_max(mask_map, neighbourhood_size=int(peak_distance),
    ...
    def TTA_postprocessing(self, img, model, transforms,
        ...
        for i in range(iterations):
            ...
            if reduction == "local_max":
                ...
                local_maxima_maps = [
                    torch_peak_local_max(mask.squeeze().float().to(device), int(peak_distance), seed_threshold,
                                         return_map=True) for mask in all_masks_list]
                local_maxima_map = torch_peak_local_max(torch.stack(local_maxima_maps).max(0)[0].squeeze(),
                                                        int(peak_distance), seed_threshold, return_map=True)
                ...
                local_maxima_map = torch_peak_local_max(all_masks.squeeze(), neighbourhood_size=int(peak_distance),
            ...
            for (t, prediction) in (zip(transforms, all_predictions)):
                ...
                out = self.postprocessing(prediction_tmp, mask_threshold, peak_distance, seed_threshold,
            ...
            labels = self.postprocessing(prediction, mask_threshold, peak_distance, seed_threshold, overlap_threshold,
        ...
        with torch.no_grad():
            ...
            for image_index in range(x_full.shape[0]):
                ...
                for i in iterations:
                    ...
                    mask_map = ((x[
                        self.dim_coords + self.n_sigma]) / 15) + 0.5  # inverse transform applied to edt during training.
                    ...
                    if precomputed_seeds is None or precomputed_seeds.shape[0] == 0:
                        centroids_idx = torch_peak_local_max(mask_map, neighbourhood_size=peak_distance,
                                                             minimum_value=seed_threshold,
                                                             dtype=self.index_dtype)  # .to(prediction.device)
                    else:
