...
from cellembed.utils.pytorch_utils import torch_sparse_onehot, torch_fastremap, remap_values, torch_onehot

def get_intersection_over_union(label: torch.Tensor, return_lab: bool = True) -> torch.Tensor:

    from cellembed.utils.pytorch_utils import fast_sparse_dual_iou, torch_sparse_onehot
...
def get_mean_object_features(image: torch.Tensor, label: torch.Tensor) -> torch.Tensor:
    ...
    if label.max() == 0:
        return torch.tensor([])
    label = label.squeeze()
    from cellembed.utils.pytorch_utils import torch_sparse_onehot
...
if __name__ == "__main__":
  ...
  #  input_tensor, _ = Augmenter.to_tensor(input_data, normalize=True)
    ...
    from cellembed.utils.loss.instanseg_loss import InstanSeg_Torchscript
