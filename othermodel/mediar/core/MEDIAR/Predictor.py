...
from othermodel.mediar.core.MEDIAR.utils import compute_masks
...
class Predictor(BasePredictor):
    ...
    def _post_process(self, pred_mask):
        ...
        # dP：方向场（位移场），从 pred_mask 的前两个通道提取。
        # cellprob：细胞概率图，由 pred_mask 的最后一个通道通过 sigmoid 函数进行归一化处理，结果为 0 到 1 之间的概率。
        dP, cellprob = pred_mask[:2], self._sigmoid(pred_mask[-1])

        # H, W：获取图像的高度（H）和宽度（W）。
        H, W = pred_mask.shape[-2], pred_mask.shape[-1]

        # 这是一个条件判断。如果图像的像素数小于 5000 * 5000，就直接进行分割处理。如果图像较小，可以直接用 compute_masks 处理。
        if np.prod(H * W) < (5000 * 5000):
            # 调用 compute_masks 函数，传入方向场 dP、细胞概率 cellprob 以及其他参数（使用 GPU、阈值等）。
            # compute_masks 函数返回的第一个元素是分割掩码，将其赋值给 pred_mask。
