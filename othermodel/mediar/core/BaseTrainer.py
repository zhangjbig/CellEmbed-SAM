...
from othermodel.mediar.core.utils import print_learning_device, print_with_logging
from othermodel.mediar.measures import evaluate_f1_score_cellseg
...
class BaseTrainer:
    ...
    # 定义了 train() 方法，该方法将用于训练模型
    def train(self):
        """Train the model"""

        # Print learning device name
        # 调用了一个辅助函数 print_learning_device，用于打印当前模型训练的设备（例如 CPU 或 GPU）。这在调试和日志记录中非常有用，可以确保模型在正确的设备上运行
        print_learning_device(self.device)

        # Learning process
        # 循环开始，从第 1 个 epoch 到指定的 self.num_epochs。epoch 是训练中的一个完整的前向和后向传播过程，循环将控制整个训练过程。
        for epoch in range(1, self.num_epochs + 1):
            ...
            # 调用 self._epoch_phase() 方法，传入参数 "train"，代表训练阶段的一个 epoch。
            # 该函数的作用是处理整个 epoch 期间的数据前向传播、损失计算和梯度更新，并返回训练结果 train_results。
            train_results = self._epoch_phase("train")

            # 调用 print_with_logging() 方法，将当前 epoch 的训练结果打印出来并记录日志。train_results 包含了训练过程中计算的各种指标，如损失、准确率等。
            print_with_logging(train_results, epoch)

            # 检查是否有学习率调度器（scheduler）。如果有调度器，它会在每个 epoch 后更新学习率
            if self.scheduler is not None:
                # 调用调度器的 step() 方法，更新优化器的学习率。调度器通常会基于当前的 epoch 或损失动态调整学习率，帮助训练过程更好地收敛。
                self.scheduler.step()

            # 检查当前的 epoch 是否到了验证频率（valid_frequency），只有当 epoch 达到指定频率时才会执行验证或调优操作。
            if epoch % self.valid_frequency == 0:
                if not self.no_valid:
                    # Valid Epoch Phase
                    print(">>> Valid Epoch")

                    # 调用 self._epoch_phase() 方法，传入 "valid"，表示验证阶段的一个 epoch，并返回验证结果 valid_results，其中包含验证的性能指标。
                    valid_results = self._epoch_phase("valid")
                    print_with_logging(valid_results, epoch)

                    # 检查验证结果中是否包含 F1 分数（Valid_F1_Score），这是一个常见的分类评估指标。如果存在，进入下一步更新最佳模型权重。
                    if "Valid_F1_Score" in valid_results.keys():
                        # 提取当前验证轮次的 F1 分数，用于与之前最好的分数进行比较。
                        current_f1_score = valid_results["Valid_F1_Score"]

                        # 调用 _update_best_model() 方法，如果当前的 F1 分数超过了之前的最佳分数，就会更新最佳模型的权重。
